...
namespace Ecommerce.Infrastructure.SecurityHelpers
{
    public class SecurityFingerprintHelper : ISecurityFingerprintHelper
    {
        ...
        public string ComputeDeviceBinding(string deviceId)
        {
            if (string.IsNullOrWhiteSpace(_settings.DeviceBindingSecret))
                throw new InvalidOperationException("AuthSecurity:DeviceBindingSecret is not configured. Use User Secrets (AuthSecurity__DeviceBindingSecret) or environment variable.");
            
            var key = Encoding.UTF8.GetBytes(_settings.DeviceBindingSecret);
            var data = Encoding.UTF8.GetBytes(deviceId ?? string.Empty);
